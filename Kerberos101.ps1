...
#region Installation
$path = "$([System.Environment]::GetFolderPath('MyDocuments'))\WindowsPowerShell\Modules\$([System.IO.Path]::GetFileNameWithoutExtension($MyInvocation.MyCommand.Path))"

if (-not (Test-Path $path))
{
    Write-Host "Unblocking file first"
    Unblock-File -Path $MyInvocation.MyCommand.Path

    Write-Host "Module is not installed yet"
    Write-Host "Creating folder '$path'"
    New-Item -Path $path -ItemType Directory | Out-Null

    $file = "$([System.Environment]::GetFolderPath('MyDocuments'))\WindowsPowerShell\Modules\$([System.IO.Path]::GetFileNameWithoutExtension($MyInvocation.MyCommand.Path))\$([System.IO.Path]::GetFileName($MyInvocation.MyCommand.Path))"
    $file = $file.Replace('ps1', 'psm1')
    
    Write-Host "Copying file '$($MyInvocation.MyCommand.Path)' to '$file'"
    Copy-Item -Path $MyInvocation.MyCommand.Path -Destination $file

    Write-Host "Script file has been copied to the personal module store ($path). Functions are available now"
}
